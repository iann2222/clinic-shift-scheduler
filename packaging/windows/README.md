...
- `application.name`／`target`／`entry_point`：執行檔與目標平台。
...
建置器會依序執行完整測試、下載並驗證固定版本字型、建立 Regular／Bold 靜態字重、執行 PyInstaller、加入匿名輸入與使用說明、使用封裝後的執行檔完成 smoke test，最後建立：
...
smoke test 會把 ZIP 解壓到 `runtime/packaging-smoke/`，移除 Conda、virtual environment 與 Python 的環境提示，並把 `PATH` 限制為 Windows 系統目錄後啟動封裝程式，避免開發環境意外補上漏掉的 DLL。驗證完畢後會刪除整個暫存解壓目錄。

## 驗收重點

最終發布仍需在沒有 Python／Conda 的乾淨 Windows 64 位元環境解壓縮測試。使用者必須能直接修改 `config.json`、把資料放入 `input/`、執行 `ClinicShiftScheduler.exe`，並在 `output/` 取得通過驗證的 JSON、Excel 與 PDF。
