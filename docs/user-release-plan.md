...
2. 在編輯器右上角「設定」調整執行與候選參數；進階使用者仍可直接編輯根目錄 `config.json`。
3. 到「執行排班」頁執行目前月份，直接查看階段、耗時、錯誤與輸出路徑。
4. 從 `output/` 取得正式 JSON、Excel、PDF，以及設定要求的候選班表。
5. 若不使用圖形介面，仍可獨立執行 `ClinicShiftScheduler.exe` 並在主控台查看相同流程。

輸入編輯器與排班執行檔共用正式文件契約，但保持獨立；編輯器以背景子程序呼叫排班執行檔，本身不載入 solver。
...
6. 從編輯器啟動獨立排班程序，確認進度持續更新、視窗可操作且可提出取消要求。
7. 使用匿名月份範本完成一次完整排班，確認結果為 `OPTIMAL` 且 validation PASS。
8. 重新開啟並人工抽查 JSON、Excel 與 PDF，確認中文、表格、統計及檔案路徑正常。
9. 測試中文路徑、含空白路徑、既有輸出、錯誤輸入與中途停止等常見情境。
10. 發布前記錄應用程式版本、Schema 版本、依賴版本與驗收結果，再產生 ZIP 或安裝程式。
