...
1. 執行 `clinic-shift-scheduler.exe` 建立、編輯、驗證並儲存 `input/` 內的月份 JSON；進階使用者仍可直接編輯 JSON。
2. 在編輯器右上角「設定」調整執行與候選參數；進階使用者仍可直接編輯根目錄 `config.json`。
3. 到「執行排班」頁執行目前月份，直接查看階段、耗時、錯誤與輸出路徑。
4. 從 `output/` 取得正式 JSON、Excel、PDF，以及設定要求的候選班表；若使用者提早停止並保留，則取得另行命名且清楚標示為 `FEASIBLE` 的暫存結果。
5. 若不使用圖形介面，仍可獨立執行 `quick-runner.exe` 並在主控台查看相同流程。
